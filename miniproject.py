...
system_prompt =f"""
you are an expert HR assitant.

your job is to analyze job descriptions and extract structured info5rmation from them.
...
fill the schema wwith the actual information extracted from the job description .
if minimum experience is not mention return null 
if any information is missing return empty list 
...
#now converting our output into a json file.
raw_json=answer 
...
def final_score(job,resume):
    ...
    prompt=f"""
    You are an HR recurier.
    compare the candidate reusme with the job description.
    JOB DESCRIPTION:
...
4.wheather experience requirement meets or not
5. overall match percentage from 0 to 100
6. what improvemnet can be done 
7. a final verdict

keep the resonse concise and easy to read.
...
extract information based on its meaning,not only based on the headings

different resume have different headings . 

for example 
-experience
-professional expeirence
-work history 
-presvious background
etc

these may all contain relevant experience
skills may also appear in the skills section or the work experince 
return only valif json format 
matching this schema:{resume_schema}
1. do not invent information.
2. if a value is not available return null.
3. if a list has no information , return an empty list.
4. inlcude internships and inside experince
